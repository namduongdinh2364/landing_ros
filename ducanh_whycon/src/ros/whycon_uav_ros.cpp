...
whycon::RobotPosePublisher::RobotPosePublisher(ros::NodeHandle& n)
{
  ...
  mavposeSub_ = n.subscribe("mavros/local_position/pose", 1, &whycon::RobotPosePublisher::mavposeCallback, this,
    ...
    pub_pose_marker_camera_frame = n.advertise<geometry_msgs::PoseStamped>("/whycon/pose/camera_frame", 10);
  ...
  case 2:
    {
        ...
        target_pose.header.stamp = ros::Time::now();
        target_pose.pose.position.x = (pos(0) + pose1(0))/2;
        target_pose.pose.position.y = (pos(1) + pose1(1))/2;
        target_pose.pose.position.z = (pos(2) + pose1(2))/2;
        pub_pose_marker_camera_frame.publish(target_pose);
    }
    break;
  case 3:
