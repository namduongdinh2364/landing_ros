...
void whycon::WhyConROS::on_image(const sensor_msgs::ImageConstPtr& image_msg, const sensor_msgs::CameraInfoConstPtr& info_msg)
{
  ...
  is_tracking = system->localize(image, should_reset/*!is_tracking*/, max_attempts, max_refine);
    ...
    std::cout << "test image callback" << std::endl;
    if (!is_tracking)
    {
        targets -= 1;
        system = boost::make_shared<whycon::LocalizationSystem>(targets, image.size().width, image.size().height, cv::Mat(camera_model.fullIntrinsicMatrix()), dis_coeff_temp, parameters);
  ...
  if (is_tracking) {
    std::cout << "test tracking" << std::endl;
...
{
  bool publish_images = true; //(image_pub.getNumSubscribers() != 0);
  bool publish_poses = true; //(poses_pub.getNumSubscribers() != 0);
...
//   if (!publish_images && !publish_poses) return;
  ...
  geometry_msgs::PoseArray pose_array;
    ...
    std::cout << "test12341213======= " << system->targets << std::endl;
