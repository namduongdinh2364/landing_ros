...
public:
	...
	: nh_(nh), it(nh_private)
	{
		...
		PoseAruco_sub = nh_.subscribe("/aruco_detector/pose", 1, &DebugImage::getPoseArucoCallback,
	...
	void image_callback(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		try
		{
			...
			for (int i = 0; i < rvecs.size(); ++i) {
				...
				cv::putText(inImage, "Hello, OpenCV!", cv::Point(10, inImage.rows / 2), cv::FONT_HERSHEY_DUPLEX, 1.0, CV_RGB(118, 185, 0), 2);

				std::cout << tvec << std::endl;
				std::cout << rvec << std::endl;
				cv::drawFrameAxes(inImage, cameraMatrix, distCoeffs, rvec, tvec, 0.1);
			}
