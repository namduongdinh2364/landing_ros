...
void transformAdaption::get_marker_pose()
{
	...
	if(!locked_landing_) {
		...
		if (cur_pose_(2) > ALTITUDE_CHANGE_METHOD + 1.0) {
			if (cur_pose_(2) > 8.5) {
				...
				desPose_.pose.position.x = markerPose_(0);
				desPose_.pose.position.y = markerPose_(1);
				desPose_.pose.position.z = ALTITUDE_CHANGE_METHOD - 1.0;
			}
...
void transformAdaption::get_marker_pose_with_distance()
{
	...
	/* Check error angle */
	float var_AC;
	var_AC = (float)sqrt(pow(markerPose_[0] - cur_pose_[0] ,2) + pow(markerPose_[1] - cur_pose_[1],2));
