	...
	locked_landing_(false)
{
	...
	sub_marker_pose_ = nh_.subscribe
		("/tf", 1, &transformAdaption::get_marker_pose_Callback, this, ros::TransportHints().tcpNoDelay());
...
void transformAdaption::loopCallback(const ros::TimerEvent& event) {
	...
	{	
		locked_trans_ = false;
		if (cur_pose_(2) > ALTITUDE_CHANGE_METHOD + 0.5) {
...
void transformAdaption::get_marker_pose_Callback(const tf2_msgs::TFMessage& msg)
{
	...
	if(!locked_landing_) {
		locked_trans_ = true;
		m_yaw_ = get_Euler_from_quat(msg.transforms[0].transform.rotation.x,
									 msg.transforms[0].transform.rotation.y,
									 msg.transforms[0].transform.rotation.z,
									 msg.transforms[0].transform.rotation.w, Axis::YAW);
		/* Marker ----> Drone */
		cam_Pose_[0] = msg.transforms[0].transform.translation.x;
		cam_Pose_[1] = msg.transforms[0].transform.translation.y;
		cam_Pose_[2] = msg.transforms[0].transform.translation.z;
...
void transformAdaption::get_marker_pose_with_distance()
{
	...
	if(var_AC <= DISTANCE) {
		...
		if (cur_pose_(2) >= 0.8) {
			if (desPose_.pose.position.z <= 0.5) {
				desPose_.pose.position.z = 0.5;
			} else {
				desPose_.pose.position.z = cur_pose_(2) -5.0;
			}
		}
		else {
			accept_landing = true;
